"""Persistent two-slot Isaac evaluator, controlled by a scheduler mailbox.

CLI options mirror eval_vector.py. The initial task arguments validate startup
configuration; actual chunks are supplied through --worker-dir/request.tsv.
"""
...
def main() -> None:
    ...
    from omnigibson.eval.persistent_vector_evaluator import PersistentVectorEvaluator
    from omnigibson.eval.utils.persistent_worker import serve_requests
    ...
    evaluator = PersistentVectorEvaluator(cfg)
    status = 1
    try:
        status = serve_requests(args.worker_dir, evaluator.run_request)
    finally:
        # SimulationApp.close may terminate the process. The mailbox response
        # has already been published, so the scheduler retains failure status.
        evaluator.close()
